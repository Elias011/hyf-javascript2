...
repar.onclick = function(){
	...
	req.onload = function result(){
		...
		for(i in data){
			nameOfRepositories.innerHTML += "<a target = '_blank' href = "+ data[i].html_url + ">" + data[i].name +"</a>" + "<br>";
		}
	};
	req.send();
}
...
searchButton.onclick = function(){
	...
	req.onload = function result(){
		...
		for(i in data){
			...
			if(data[i].name == x){
				nameOfRepositories.innerHTML = "<a target = '_blank' href = "+ data[i].html_url + ">" + data[i].name + "</a>" + "<br>" + "Stragazers: " + data[i].stargazers_count + "<br>" + "Watchers:" + data[i].watchers + "<br>" + "Fork: " + data[i].forks_count + "<br>" + "Language:" + data[i].language;
			}
		}
	};
	req.send();
